...
tags:
  ...
  - chememo/hub
  - moc
next_action: "TIER 0 FULLY SHIPPED on dev as of 2026-07-28 — all 11 items (T0.1–T0.11) done and verified. University network block resolved; T0.10's final live check ran clean but surfaced three real bugs in the health-metrics code (stale null-model job winning the 'latest' pick; indexedCount exceeding totalExperiments; indexedCount sourced from index_jobs instead of experiment_embeddings, undercounting pre-T0.5 seed experiments) — all three found and fixed same session, verified live against the real dev URL and cross-checked against direct SQL (final state: gemini-embedding-001/1536, 14/14 experiments indexed). T0.11 (protect seed/reference project deletion) also shipped 2026-07-28: tightened projects_delete_own RLS to owner-only; found dev still had 4 ownerless seed projects prod had already cleaned up, backfilled their owner_id to yishieze@gmail.com per user decision, verified via a direct RLS test. Nothing promoted to master/prod as app code yet — all Tier 0 work is dev-only, per the plan's tier-by-tier gate. See [[ChemMemo_Product_Evolution_Plan]] Tier 0 and [[ChemMemo_Implementation_Plan]] session history (2026-07-26/27/28 entries) for the full account of T0.6–T0.11. Next: Tier 0 is done — reassess with Yishi before starting Tier 1 (make it a daily lab notebook), per the plan's explicit gate. Do NOT start Tiers 1–4 without explicit direction, and consider whether/when to promote the accumulated dev-only Tier 0 work to master first."
updated: 2026-07-28
---
