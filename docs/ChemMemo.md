...
tags:
  ...
  - chememo/hub
  - moc
next_action: "T0.1–T0.6 ALL fully shipped on dev (T0.1–T0.4 on 2026-07-23, T0.5 on 2026-07-25, T0.6 completed 2026-07-26). T0.6 includes a full CI pipeline (unit+component tests, Playwright E2E+axe, RLS integration tests, migration validation, dependency audit, secret scan) CONFIRMED FULLY GREEN in GitHub Actions, plus branch protection now LIVE on master (requires ci+rls checks; future dev→master promotions likely need a GitHub PR rather than a raw push — see plan/session-history for why). See [[ChemMemo_Product_Evolution_Plan]] Tier 0 and [[ChemMemo_Implementation_Plan]] session history for the full account. NOT yet promoted to master/prod as app code (all six items are dev-only). Next: T0.7 (generate DB types from Supabase schema) — the next unstarted Tier 0 item per the plan's own ordering. One Tier 0 item at a time; do not batch. Do NOT start Tiers 1–4 without explicit direction."
updated: 2026-07-26
---
...
- 🟡 **In progress:** [[ChemMemo_Product_Evolution_Plan]] Tier 0 — **T0.1–T0.6 all fully shipped on dev** (T0.1–T0.4 on 2026-07-23, T0.5 on 2026-07-25, T0.6 on 2026-07-26; T0.2 closed a real stored-XSS hole, T0.4 closed a provenance risk, T0.5 is spec-first — [[ChemMemo_Feature_IndexJobs_Spec]]). T0.6 (test + CI baseline) is now **complete**: Vitest unit+component tests, Playwright E2E + axe, Supabase RLS integration tests, and a full CI pipeline (migration validation, dependency audit, secret scan) are all confirmed fully green on GitHub Actions, and branch protection is live on `master`. Not yet promoted to master/prod as app code. Next: T0.7.
