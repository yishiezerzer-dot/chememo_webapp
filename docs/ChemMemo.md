...
tags:
  ...
  - chememo/hub
  - moc
next_action: "T0.1–T0.5 fully shipped on dev; T0.6 PARTIAL but substantial: Vitest unit tests, Playwright E2E (7 tests via a dedicated e2e-test@chememo.local account), axe accessibility checks, and a full CI pipeline are all done and CONFIRMED FULLY GREEN on GitHub Actions (commit 9241b64, 1m34s, every stage passed). See [[ChemMemo_Product_Evolution_Plan]] Tier 0 and [[ChemMemo_Implementation_Plan]] session history (2026-07-25/26) for the full account. Still missing for T0.6: component tests, more E2E flows, Supabase RLS integration tests (needs Docker — not available on this dev machine, untried in CI), migration-validation/dependency-audit/secret-scan CI stages, and branch protection on master (needs explicit confirmation first). NOT yet promoted to master/prod. Next: continue T0.6's remaining pieces OR move to T0.7+ — check with the user. One Tier 0 item at a time; do not batch. Do NOT start Tiers 1–4 without explicit direction."
updated: 2026-07-26
---
...
- 🟡 **In progress:** [[ChemMemo_Product_Evolution_Plan]] Tier 0 — T0.1–T0.5 fully shipped on **dev** (T0.1–T0.4 on 2026-07-23, T0.5 on 2026-07-25; T0.2 closed a real stored-XSS hole, T0.4 closed a provenance risk, T0.5 is spec-first — [[ChemMemo_Feature_IndexJobs_Spec]]). T0.6 (test + CI baseline) is **substantially built, still partial**: Vitest unit tests, Playwright E2E (7 tests, dedicated test account), axe accessibility checks, and the full CI pipeline are all **confirmed fully green on GitHub Actions**. Missing: component tests, more E2E flows, RLS integration tests (Docker-blocked locally), remaining CI stages, branch protection. Not yet promoted to master/prod.
