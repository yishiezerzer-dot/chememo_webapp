...
status: active
priority: high
next_action: Continuing the audit's P0 sprint sequence ([[ChemMemo_Audit_Roadmap]]). Sprint S1 (mobile nav + global search) done 2026-07-16. Sprint S2 (embedding sync on save + EXP-ID DB sequence) done 2026-07-16 on dev — migration applied to chememo-dev only, seeded at EXP-013. Only remaining P0 is Sprint S6 (prod promotion — needs explicit confirmation before running, production-deploy stakes). Also open (P1/P2): dynamic sidebar project filters (S3), toasts + error/loading boundaries (S4), retrieval eval set (S5). AI is LIVE on dev via Gemini (switchable to openai/anthropic via AI_PROVIDER). (Reminder: chememo-dev auto-pauses after ~8 days idle — restore_project first.)
---
...
- **Last session:** 2026-07-16 — **Audit P0 Sprints S1 + S2 shipped to dev.** S1: mobile nav + working global search + subdued backgrounds (commit `d159b6c`). S2: embedding auto-sync on save (`lib/sync-embedding.ts`, fire-and-forget from `new/actions.ts`) + atomic EXP-ID DB sequence (migration `20260716120000`, `lib/experiment-id.ts`); migration applied to **chememo-dev only** (seeded at EXP-013), commit `a65bcbb`, deploy SUCCESS, routes 307. Remaining P0: only Sprint S6 (prod promotion — needs go-ahead). Manual check still owed: create an experiment on dev and confirm a fresh `experiment_embeddings` row appears.
